...
################################################################################
################################################################################
## 8) MAPA DE EMPRENDEDORES (%) – QUINTILES DISCRETOS (SIN ERRORES)
################################################################################

# porcentaje
inter$prop_pct <- 100 * inter$prop_empr

# 1) Calcular cortes por cuantiles (5 clases)
brks_q <- classInt::classIntervals(
  inter$prop_pct[is.finite(inter$prop_pct)],
  n     = 5,
  style = "quantile"
)$brks

# 2) Crear variable categórica (quintiles)
inter$clase_empr <- cut(
  inter$prop_pct,
  breaks = brks_q,
  include.lowest = TRUE,
  dig.lab = 4
...
# 3) Paleta discreta (5 tonos)
pal_disc <- pal_blues[c(1, 3, 4, 5, 7)]

p_empr <- ggplot(inter) +
  geom_sf(aes(fill = clase_empr),
  ...
  labs(
    title    = "Población con actividad emprendedora (%)",
...
ggsave(
  "mapa_emprendedores_urbano_quintiles.png",
  p_empr, width = 9, height = 7, dpi = 300
)
...
# opcional: todos juntos
plot_grid(g_empr_casas, g_empr_prof, g_casas_prof, ncol = 3)
...
p_shannon <- ggplot(gs_comunas_shannon) +
  ...
  labs(
    title    = "Diversidad intra-comunal de perfiles de emprendimiento",
